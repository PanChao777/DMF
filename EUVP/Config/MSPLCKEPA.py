...
class MixStructureBlock(nn.Module):
    def __init__(self, dim):
        ...
        # 深度扩张卷积组（Dilation=3）
        self.conv3_19 = nn.Conv2d(dim, dim, kernel_size=7, padding=9, 
                                groups=dim, dilation=3, padding_mode='reflect')
        self.conv3_13 = nn.Conv2d(dim, dim, kernel_size=5, padding=6,
                                groups=dim, dilation=3, padding_mode='reflect')
        self.conv3_7 = nn.Conv2d(dim, dim, kernel_size=3, padding=3,
                               groups=dim, dilation=3, padding_mode='reflect')

        # 特征融合MLP
        self.mlp = nn.Sequential(
            nn.Conv2d(dim * 3, dim * 4, 1),  # 3分支concat后升维
            nn.GELU(),
            nn.Conv2d(dim * 4, dim, 1)       # 降维回原始通道数
        )
    ...
    def forward(self, x):
        ...
        # 并行深度扩张卷积分支
        x = torch.cat([
            self.conv3_19(x),      # kernel7-dilation3
            self.conv3_13(x),      # kernel5-dilation3 
            self.conv3_7(x)        # kernel3-dilation3
        ], dim=1)                  # 通道维度拼接
        x = self.mlp(x)            # 特征融合
